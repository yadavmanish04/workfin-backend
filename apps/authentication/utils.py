...
def send_otp_email(email, otp):
    subject = 'WorkFina - Email Verification Code'
    logging.info(f'[OTP DEBUG] Sending OTP {otp} to {email}')
    ...
    try:
        ...
        logging.info(f'[OTP DEBUG] OTP successfully sent to {email}')
    except Exception as e:
        logging.error(f'[OTP ERROR] Failed to send OTP: {e}', exc_info=True)
        raise
